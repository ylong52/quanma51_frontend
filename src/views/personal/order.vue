  ...
  <main class="min-h-screen bg-gray-100 flex flex-col ">
    <div class="w-full max-w-md mx-auto flex-1 pb-20"> <!-- 给底部留空间 -->
      ...
      <div class="flex items-center px-2 py-2 bg-white border-b border-gray-100 sticky top-0 z-10">
        ...
        <div class="flex-1 flex items-center bg-gray-100 rounded-full px-2 py-1">
          ...
          <input v-model="searchKeyword" @keyup.enter="searchOrders" type="text" placeholder="所有单号/商品名称/充值账号/卡密" class="flex-1 bg-transparent border-0 outline-none text-sm px-1" />
        </div>
      ...
      <!-- tabs -->
      <div class="flex justify-between items-center bg-white px-2 pt-2 pb-1 border-b border-gray-100 sticky top-[48px] z-10">
        <button v-for="tab in [
          {label:'全部', name:'all'},
          {label:'待付款', name:'0'},
          {label:'待处理', name:'wait'},
          {label:'处理中', name:'doing'},
          {label:'已完成', name:'1'},
      ...
      <div class="px-2 py-2">
        <div class="space-y-4">
          ...
          <!-- 订单卡片 -->
          <div v-else v-for="order in filteredOrders" :key="order.id" class="rounded-xl bg-white shadow border border-gray-100 overflow-hidden">
            ...
            <div class="flex justify-between items-center px-4 py-2">
              <div class="text-base font-medium">
                总计：<span class="text-blue-500 font-bold">¥{{ order.should_amount }}</span>
              </div>
            </div>
          </div>
        </div>
    ...
    <div class="fixed left-0 right-0 bottom-0 bg-white border-t border-gray-200 z-20 max-w-md mx-auto w-full">
      <el-pagination
        ...
        class="bg-white rounded shadow px-2"
        :background="true"
        :small="true"
      />
    </div>
