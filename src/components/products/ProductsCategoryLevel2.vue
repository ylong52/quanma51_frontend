<template>
  <div class="bg-white rounded-xl shadow-sm p-4">
    ...
    <div class="flex flex-wrap gap-6 justify-start">
      ...
      <div
        v-for="service in productCategoryLevel2"
        :key="service.id"
        :id="`${service.id}Btn`"
      ...
      >
        <div class="w-14 h-14 flex items-center justify-center rounded-full bg-neutral-100 mb-2 overflow-hidden">
          <template v-if="service.img">
            <img :src="service.img" :alt="service.category" class="w-12 h-12 object-contain" />
          </template>
          <template v-else>
        ...
        <span class="text-xs font-medium text-neutral-700 mt-1">{{ service.category }}</span>
      </div>
    </div>
  </div>
</template>
...
onMounted(async () => {
  await getProductCategoryLevel2();
});

const productCategoryLevel2 = ref([]);

const getProductCategoryLevel2 = async () => {
  const res = await api.getProductCategoryLevel2();
  productCategoryLevel2.value = res.data;
}
